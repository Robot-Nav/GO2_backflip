...
class BackflipConfig:
    def __init__(self, file_path, actor_override=None):
        ...
        self.joint_target_limit_margin = float(
            cfg.get("joint_target_limit_margin", 0.08)
        )
    ...
    def _validate(self):
        ...
        if self.joint_target_limit_margin < 0.0:
            raise ValueError("joint_target_limit_margin 不能为负数")
        if np.any(
            self.joint_lower_limits + self.joint_target_limit_margin
            >= self.joint_upper_limits - self.joint_target_limit_margin
        ):
            raise ValueError("关节位置范围不足以容纳 joint_target_limit_margin")
