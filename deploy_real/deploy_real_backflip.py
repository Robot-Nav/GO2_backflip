...
A trigger resets the policy phase and action-history state exactly as at the
start of a training episode. It does not reset the physical robot. The actor
continuously controls takeoff, landing, recovery and the waiting stance.
"""
...
class BackflipController:
    ...
    def step(self):
        ...
        if self.remote.is_pressed(KeyMap.A):
            ...
            if ready:
                self.phase_step = 0
                # Training clears these policy-owned states at episode start.
                # Preserve the measured robot state, but do not feed stale
                # waiting-stance actions into the newly triggered flip.
                self.current_action.fill(0.0)
                self.last_action.fill(0.0)
                self.slew_limited_action.fill(0.0)
                self.backflip_active = True
                print(
                    f"[A] BACKFLIP triggered at control step {self.counter}; "
        ...
        target = np.clip(
            target,
            self.config.joint_lower_limits + self.config.joint_target_limit_margin,
            self.config.joint_upper_limits - self.config.joint_target_limit_margin,
        )
