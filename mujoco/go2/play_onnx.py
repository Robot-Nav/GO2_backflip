...
The policy owns the complete trajectory. SPACE restarts the policy phase and
its action-history state to match an Isaac Lab episode start; it never
teleports or resets the robot. After the two-second backflip phase the phase is
frozen at its final value while the actor keeps recovering/standing. Only R
performs an explicit physical reset.
"""
...
# Original Gym deployment sent its raw PD target to MuJoCo.  Safety-stage
# testing may opt in to an inner limit margin using --joint-target-margin.
...
OBS_CLIP = 100.0
ACTION_CLIP = 100.0
# With the training URDF converted to MJCF, a correctly standing Go2 settles
# at about 0.232 m base height (rather than the 0.32-m reset coordinate).
# Keep the manual-trigger guard below that value while tilt and velocity guards
# continue to reject a fallen or moving robot.
READY_MIN_BASE_HEIGHT = 0.22
...
def make_parser():
    ...
    parser.add_argument(
        "--joint-target-margin", type=float, default=JOINT_TARGET_LIMIT_MARGIN,
        help="PD 目标相对关节硬限位的内缩量 [rad]；第一阶段 Gym 对齐使用 0。",
    )
    parser.add_argument(
...
def main():
    ...
    if args.joint_target_margin < 0.0:
        raise ValueError("--joint-target-margin 不能小于 0")
    ...
    # The current policy was trained for one flip per physical episode.  Do
    # not silently treat phase reset as a second trained maneuver.
    flip_attempted_since_reset = backflip_active
    policy_step = 0
    next_auto_trigger = (
    ...
    max_joint_limit_joint = ""
    max_joint_limit_position = 0.0
    max_joint_limit_bound = 0.0
    head_contact = False
    ...
    print(f"Joint target margin: {args.joint_target_margin:g} rad")
    print(
        "Joint dynamics : "
        ...
        with viewer_context as viewer:
            while viewer.is_running() and keyboard_state.running:
                ...
                if reset_requested:
                    ...
                    flip_attempted_since_reset = False
                ...
                if (manual_trigger or auto_trigger) and not backflip_active:
                    ...
                    trigger_gravity, trigger_ang_vel = projected_gravity_and_body_ang_vel(
                        data, base_body_id
                    )
                    trigger_tilt = float(
                        np.arccos(np.clip(-trigger_gravity[2], -1.0, 1.0))
                    )
                    trigger_joint_speed = float(
                        np.max(np.abs(data.qvel[dof_addresses]))
                    )
                    if flip_attempted_since_reset:
                        print(
                            f"[{source}] trigger rejected: this policy is trained for "
                            "one flip per physical episode; press R before another attempt"
                        )
                    elif (
                        trigger_tilt > 0.25
                        or float(data.qpos[2]) < READY_MIN_BASE_HEIGHT
                        or abs(float(trigger_ang_vel[1])) > 1.0
                        or trigger_joint_speed > 2.0
                    ):
                        print(
                            f"[{source}] trigger rejected: robot is not ready "
                            f"(tilt={trigger_tilt:.2f}, base_z={data.qpos[2]:.3f}, "
                            f"pitch_rate={trigger_ang_vel[1]:.2f}, "
                            f"joint_speed={trigger_joint_speed:.2f})"
                        )
                    else:
                        # Match the training reset for policy-owned state while
                        # leaving the physical MuJoCo state untouched.
                        phase_time = 0.0
                        current_action.fill(0.0)
                        last_action.fill(0.0)
                        slew_limited_action.fill(0.0)
                        backflip_active = True
                        flip_attempted_since_reset = True
                        print(
                            f"[{source}] backflip triggered at sim t={data.time:.2f}s; "
                            "policy action history reset"
                        )
                ...
                for _ in range(DECIMATION):
                    ...
                    if args.joint_target_margin > 0.0:
                        target_pos = np.clip(
                            raw_target_pos,
                            joint_ranges[:, 0] + args.joint_target_margin,
                            joint_ranges[:, 1] - args.joint_target_margin,
                        )
                    else:
                    ...
                    limit_index = int(np.argmax(limit_excess))
                    step_limit_excess = float(max(limit_excess[limit_index], 0.0))
                    if step_limit_excess > max_joint_limit_excess:
                        max_joint_limit_excess = step_limit_excess
                        max_joint_limit_joint = JOINT_NAMES[limit_index]
                        max_joint_limit_position = float(step_dof_pos[limit_index])
                        if step_dof_pos[limit_index] < joint_ranges[limit_index, 0]:
                            max_joint_limit_bound = float(joint_ranges[limit_index, 0])
                        else:
                            max_joint_limit_bound = float(joint_ranges[limit_index, 1])
    ...
    hardware_safety_pass = (
        max_joint_limit_excess <= 1.0e-3
        and max_abs_joint_speed <= 31.5
    )
    validation_pass = task_pass and hardware_safety_pass
    print(
    ...
    print(
        ...
        f"({max_joint_limit_joint}: {max_joint_limit_position:.4f}, "
        f"边界 {max_joint_limit_bound:.4f}) | "
        f"头部接触 {'是' if head_contact else '否'}"
    )
    print(f"任务验证：{'PASS' if task_pass else 'FAIL'}")
    print(f"硬件安全验证：{'PASS' if hardware_safety_pass else 'FAIL'}")
    print(f"总体验证：{'PASS' if validation_pass else 'FAIL'}")
