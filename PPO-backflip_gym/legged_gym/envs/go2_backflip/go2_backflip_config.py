...
class Go2BackflipCfg(Go2BaseCfg):
    ...
    class control(Go2BaseCfg.control):
        ...
        # at each joint's velocity limit below. Y1/Y2 are drive/brake limits.
        motor_velocity_x1 = 13.5
        motor_torque_y1 = 20.2
        motor_torque_y2 = 23.4
        # Use the Go2HV actuator no-load speed used by the project for all
        # twelve joints, including the calf motors.
        joint_velocity_limits = [30.0] * 12
        target_velocity_limits = [13.5] * 12
        velocity_termination_start_ratio = 1.50
        velocity_termination_ratio = 1.00
        # Position termination uses the original URDF range.  Early training
        # gets room to rediscover the original motion; the margin reaches zero
        # with the safety curriculum.
        position_termination_start_margin = 0.30
        position_termination_margin = 0.0
        position_validation_tolerance = 1.0e-3
    ...
    class domain_rand(Go2BaseCfg.domain_rand):
        ...
        added_center_range = [-0.015, 0.015]
    ...
    class rewards(Go2BaseCfg.rewards):
        ...
        rotation_end = 1.00
        landing_start = 1.40
        recovery_velocity_start = 1.00
        landing_impact_window_steps = 6
        success_hold_time_s = 0.20
        success_max_joint_speed = 2.0
        unsafe_body_contact_force = 100.0
        unsafe_contact_termination_curriculum = 1.00
        max_pitch_rate = 7.2
        ...
        # Require a sustained, genuine takeoff before rotation can count.
        takeoff_confirm_steps = 2
        takeoff_min_height = 0.25
        # MuJoCo rejects any head-ground contact.  Keep this threshold low so
        # a light contact cannot be reported as a successful backflip.
        head_contact_success_force = 1.0
        body_contact_force = 100.0
        max_body_contact_penalty = 5.0
        min_head_center_height = 0.08
        head_contact_force = 40.0
        landing_force_threshold = 200.0
        max_landing_impact_penalty = 5.0
        # 24 environment steps are collected per PPO iteration. Preserve the
        # original objective for 1000 iterations, tighten through iteration
        # 4000, then leave 2000 iterations at the complete safety envelope.
        safety_curriculum_start = 0.0
        safety_curriculum_warmup_steps = 24000
        safety_curriculum_ramp_steps = 72000

        class scales:
            # Larger than completion + success individually, so terminating
            # unsafely cannot be a profitable shortcut.
            termination = -1000.0
            ang_vel_y = 5.0
            ang_vel_z = -1.0
            lin_vel_z = 20.0
            ...
            # Safety terms are curriculum-scaled by the task implementation.
            head_clearance = -5.0
    ...
    class normalization(Go2BaseCfg.normalization):
        # Joint targets are also clamped to the soft URDF range in the task.
        clip_actions = 8.0
