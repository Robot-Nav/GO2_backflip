...
class Go2Backflip(Go2):
    ...
    def _init_custom_buffers__(self):
        ...
        self.target_velocity_limits = torch.tensor(
            self.cfg.control.target_velocity_limits,
            dtype=torch.float,
            device=self.device,
        ).unsqueeze(0)
        if self.joint_velocity_limits.shape != (1, self.num_actions):
            raise ValueError("joint_velocity_limits must contain one value per action")
        if self.target_velocity_limits.shape != (1, self.num_actions):
            raise ValueError("target_velocity_limits must contain one value per action")
        if torch.any(
            self.joint_velocity_limits <= self.cfg.control.motor_velocity_x1
        ):
            raise ValueError(
                "every joint velocity limit must exceed motor_velocity_x1"
            )
        ...
        self.was_airborne = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.airborne_counter = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self.head_contact_episode = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.landing_detected = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.just_landed = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.landing_impact_steps_remaining = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self.success_hold_counter = torch.zeros(
            self.num_envs, dtype=torch.long, device=self.device
        )
        self.unsafe_episode = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.unsafe_contact_buf = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.position_violation_buf = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        # MuJoCo validates peaks at 200 Hz, not only at the 50-Hz actor rate.
        self.substep_velocity_violation = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.substep_position_violation = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self.episode_peak_speed_ratio = torch.zeros(
            self.num_envs, dtype=torch.float, device=self.device
        )
        self.episode_peak_position_excess = torch.zeros(
            self.num_envs, dtype=torch.float, device=self.device
        )
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        # Isaac stores the configured 95% soft reward range in dof_pos_limits.
        # Reconstruct the original URDF range for hard safety validation.
        soft_fraction = self.cfg.rewards.soft_dof_pos_limit
        position_center = torch.mean(self.dof_pos_limits, dim=1)
        soft_half_range = 0.5 * (
            self.dof_pos_limits[:, 1] - self.dof_pos_limits[:, 0]
        )
        hard_half_range = soft_half_range / soft_fraction
        self.hard_dof_pos_limits = torch.stack(
            (
                position_center - hard_half_range,
                position_center + hard_half_range,
            ),
            dim=1,
        )
    ...
    def reset_idx(self, env_ids):
        ...
        finished_unsafe = self.unsafe_episode[env_ids].float().clone()
        finished_airborne = self.was_airborne[env_ids].float().clone()
        finished_head_contact = self.head_contact_episode[
            env_ids
        ].float().clone()
        finished_speed_ratio = self.episode_peak_speed_ratio[env_ids].clone()
        finished_position_excess = self.episode_peak_position_excess[
            env_ids
        ].clone()
        finished_velocity_termination = getattr(
            self, "velocity_termination_buf", torch.zeros_like(self.reset_buf)
        )[env_ids].float().clone()
        finished_position_termination = getattr(
            self, "position_termination_buf", torch.zeros_like(self.reset_buf)
        )[env_ids].float().clone()
        finished_contact_termination = getattr(
            self, "contact_termination_buf", torch.zeros_like(self.reset_buf)
        )[env_ids].float().clone()
        ...
        self.extras["episode"]["unsafe_episode"] = torch.mean(finished_unsafe)
        self.extras["episode"]["airborne_episode"] = torch.mean(
            finished_airborne
        )
        self.extras["episode"]["head_contact_episode"] = torch.mean(
            finished_head_contact
        )
        self.extras["episode"]["peak_speed_ratio"] = torch.mean(
            finished_speed_ratio
        )
        self.extras["episode"]["peak_position_excess_rad"] = torch.mean(
            finished_position_excess
        )
        self.extras["episode"]["velocity_termination"] = torch.mean(
            finished_velocity_termination
        )
        self.extras["episode"]["position_termination"] = torch.mean(
            finished_position_termination
        )
        self.extras["episode"]["contact_termination"] = torch.mean(
            finished_contact_termination
        )
        ...
        self.airborne_counter[env_ids] = 0
        self.head_contact_episode[env_ids] = False
        self.landing_detected[env_ids] = False
        self.just_landed[env_ids] = False
        self.landing_impact_steps_remaining[env_ids] = 0
        self.success_hold_counter[env_ids] = 0
        self.unsafe_episode[env_ids] = False
        self.unsafe_contact_buf[env_ids] = False
        self.position_violation_buf[env_ids] = False
        self.substep_velocity_violation[env_ids] = False
        self.substep_position_violation[env_ids] = False
        self.episode_peak_speed_ratio[env_ids] = 0.0
        self.episode_peak_position_excess[env_ids] = 0.0
    ...
    def _compute_torques(self, actions):
        # Called before each 5-ms simulation step.  At this point the DOF
        # tensor contains the state produced by the preceding physics step.
        ...
        unclipped_joint_pos_target = (
            actions_scaled + self.default_dof_pos + self.motor_offsets
        )
        # The actor may ask for large targets while discovering the motion, but
        # the simulated and deployed controller must never command through a
        # mechanical stop.  dof_pos_limits already contain the configured 95%
        # URDF safety range.
        ...
        x2 = self.joint_velocity_limits * velocity_scale
    ...
    def _record_substep_safety(self):
        ...
        self.episode_peak_speed_ratio = torch.maximum(
            self.episode_peak_speed_ratio, speed_ratio
        )
        velocity_ratio = (
        ...
        self.substep_velocity_violation |= speed_ratio > velocity_ratio

        below = torch.clamp(
            self.hard_dof_pos_limits[:, 0] - self.dof_pos, min=0.0
        )
        above = torch.clamp(
            self.dof_pos - self.hard_dof_pos_limits[:, 1], min=0.0
        )
        position_excess = torch.max(below + above, dim=1).values
        self.episode_peak_position_excess = torch.maximum(
            self.episode_peak_position_excess, position_excess
        )
        position_margin = (
        ...
        self.substep_position_violation |= position_excess > position_margin
    ...
    def post_physics_step(self):
        ...
        self.airborne_counter = torch.where(
            airborne_now,
            self.airborne_counter + 1,
            torch.zeros_like(self.airborne_counter),
        )
        confirmed_takeoff = (
            (self.airborne_counter >= self.cfg.rewards.takeoff_confirm_steps)
            & (self.root_states[:, 2] >= self.cfg.rewards.takeoff_min_height)
        )
        self.was_airborne |= confirmed_takeoff
        self.just_landed = (
            self.was_airborne & (~self.landing_detected) & any_foot_contact
        )
        self.landing_detected |= self.just_landed
        ...
        self.head_contact_episode |= (
            head_force > self.cfg.rewards.head_contact_success_force
        )

        # Count signed backward rotation only before the first landing while
        # all feet are airborne.  Before takeoff confirmation the first two
        # samples are provisional and are discarded if contact immediately
        # returns, so a contact glitch or ground rocking cannot earn progress.
        ...
        invalid_pre_takeoff = (~self.was_airborne) & (~airborne_now)
        self.flip_angle[invalid_pre_takeoff] = 0.0
        self.max_flip_angle[invalid_pre_takeoff] = 0.0
        rotation_active = (
            airborne_now
            & (~self.landing_detected)
            & (~self.head_contact_episode)
        )
        self.flip_angle += pitch_rate * self.dt * rotation_active.float()
        ...
        impact_window = int(self.cfg.rewards.landing_impact_window_steps)
        self.landing_impact_steps_remaining = torch.where(
            self.just_landed,
            torch.full_like(self.landing_impact_steps_remaining, impact_window),
            torch.clamp(self.landing_impact_steps_remaining - 1, min=0),
        )

        penalized_forces = torch.norm(
            self.contact_forces[:, self.penalised_contact_indices, :], dim=-1
        )
        self.unsafe_contact_buf = (
            torch.max(penalized_forces, dim=1).values
            > self.cfg.rewards.unsafe_body_contact_force
        )
        self.position_violation_buf = self.substep_position_violation.clone()
        hard_velocity_violation = self.substep_velocity_violation.clone()
        curriculum = self._safety_curriculum_scale()
        enforce_body_contact = (
            curriculum
            >= self.cfg.rewards.unsafe_contact_termination_curriculum
        )
        unsafe_now = (
            (self.unsafe_contact_buf & enforce_body_contact)
            | self.position_violation_buf
            | hard_velocity_violation
            | self.head_contact_episode
        )
        self.unsafe_episode |= unsafe_now
        # Keep the episode metric honest if a policy becomes unsafe after it
        # briefly earned the sustained-success event.
        self.flip_success &= ~unsafe_now
        pose_error = torch.max(
            torch.abs(self.dof_pos - self.default_dof_pos), dim=1
        ).values
        max_joint_speed = torch.max(torch.abs(self.dof_vel), dim=1).values
        success_candidate = (
            self.flip_completed
        ...
        required_hold_steps = max(
            1, int(round(self.cfg.rewards.success_hold_time_s / self.dt))
        )
        success_now = self.success_hold_counter >= required_hold_steps
    ...
    def check_termination(self):
        ...
        self.velocity_termination_buf = self.substep_velocity_violation.clone()
        enforce_contact_and_position = (
            curriculum
            >= self.cfg.rewards.unsafe_contact_termination_curriculum
        )
        self.position_termination_buf = self.position_violation_buf.clone()
        self.contact_termination_buf = (
            self.unsafe_contact_buf & enforce_contact_and_position
        )
        self.reset_buf = (
            self.time_out_buf
            | self.velocity_termination_buf
            | self.position_termination_buf
            | self.contact_termination_buf
        )
    ...
    def _reward_height_control(self):
        ...
        active = (self._time() < 0.4) | self.landing_detected
        return value * active
    ...
    def _reward_landing_impact(self):
        ...
        active = self.landing_impact_steps_remaining > 0
        return (
    ...
    def _recovery_active(self):
        return self.flip_completed & self.landing_detected
    ...
    def _reward_recovery_dof_velocity(self):
        ...
        active = self._time() >= self.cfg.rewards.recovery_velocity_start
        return (
            self._safety_curriculum_scale()
