...
class BackflipController:
    def __init__(
        self, config, policy, sdk, verbose=False, allow_backflip=True
    ):
        self.config = config
        self.policy = policy
        self.sdk = sdk
        self.verbose = verbose
        self.allow_backflip = allow_backflip
        ...
        self.flip_triggered_once = False
        self.trigger_ready_since = None
        self.trigger_ready_announced = False
    ...
    def startup(self):
        ...
        for _ in range(count):
            ...
            time.sleep(self.config.control_dt)
        self._read_joint_state()
        joint_error = self.q - self.config.default_angles
        abs_joint_error = np.abs(joint_error)
        error_ratio = (
            abs_joint_error / self.config.startup_joint_error_limits
        )
        worst_joint_index = int(np.argmax(error_ratio))
        max_abs_error = float(np.max(abs_joint_error))
        max_speed = float(np.max(np.abs(self.dq)))
        print(
            f"[STARTUP] measured q={np.array2string(self.q, precision=3)}"
        )
        print(
            "[STARTUP] target   q="
            f"{np.array2string(self.config.default_angles, precision=3)}"
        )
        print(
            "[STARTUP] q error    ="
            f"{np.array2string(joint_error, precision=3)}"
        )
        print(f"[STARTUP] max joint speed={max_speed:.3f} rad/s")
        if np.any(error_ratio > 1.0):
            joint = POLICY_JOINT_NAMES[worst_joint_index]
            raise SafetyStop(
                f"默认站姿未收敛: {joint} 实际="
                f"{float(self.q[worst_joint_index]):.3f}, 目标="
                f"{float(self.config.default_angles[worst_joint_index]):.3f}, "
                f"误差={float(joint_error[worst_joint_index]):+.3f} rad "
                f"(|误差|门限 "
                f"{float(self.config.startup_joint_error_limits[worst_joint_index]):.3f} rad)"
            )
        ...
        print(f"[STARTUP] max joint error={max_abs_error:.3f} rad")
    ...
    def enable_policy(self):
        self._read_joint_state()
        # Blend from the position target that was already supporting the
        # robot, not from measured q. Starting at measured q would make the
        # PD error (and therefore gravity-support torque) abruptly vanish.
        with self.command_lock:
            self.enable_pose = self.last_target.copy()
        ...
        with self.command_lock:
            ...
            self.last_target_update_time = time.monotonic()
        print("[POLICY] direct torque enabled at terminal phase; waiting for A")
    ...
    def safety_check(self, projected_gravity=None):
        ...
        speed_limits = (
            ...
            * self.config.velocity_safety_ratio
        )
        speed_ratio = np.abs(self.dq) / speed_limits
        if np.any(speed_ratio > 1.0):
            joint_index = int(np.argmax(speed_ratio))
            raise SafetyStop(
                f"关节速度过大: {POLICY_JOINT_NAMES[joint_index]} "
                f"{abs(float(self.dq[joint_index])):.1f} > "
                f"{float(speed_limits[joint_index]):.1f} rad/s"
            )
        below = self.q < (
            self.config.joint_lower_limits
            - self.config.joint_position_tolerance
        )
        above = self.q > (
            self.config.joint_upper_limits
            + self.config.joint_position_tolerance
        )
        if np.any(below | above):
            joint_index = int(np.flatnonzero(below | above)[0])
            raise SafetyStop(
                f"关节位置越界: {POLICY_JOINT_NAMES[joint_index]} "
                f"q={float(self.q[joint_index]):.3f} rad"
            )
    def _instant_trigger_ready(self, projected_gravity):
        if self.config.single_flip_only and self.flip_triggered_once:
            return False, "one-shot backflip has already been triggered"
        if self.backflip_active:
            return False, "previous phase is still active"
        if self.policy_enabled_at is None:
            return False, "policy is not enabled"
        blend_elapsed = time.monotonic() - self.policy_enabled_at
        if blend_elapsed < self.config.blend_in_s:
            return False, (
                f"policy blend-in {blend_elapsed:.2f}/"
                f"{self.config.blend_in_s:.2f}s"
            )
        ...
        reference_error = np.abs(
            self.q - self.config.trigger_joint_reference
        )
        reference_ratio = (
            reference_error
            / self.config.trigger_joint_reference_tolerances
        )
        if np.any(reference_ratio > 1.0):
            joint_index = int(np.argmax(reference_ratio))
            return False, (
                f"terminal pose {POLICY_JOINT_NAMES[joint_index]} error "
                f"{float(reference_error[joint_index]):.3f} > "
                f"{float(self.config.trigger_joint_reference_tolerances[joint_index]):.3f} rad"
            )
    ...
    def _update_trigger_readiness(self, projected_gravity):
        """Require every trigger condition to remain true continuously."""
        instant_ready, reason = self._instant_trigger_ready(projected_gravity)
        now = time.monotonic()
        if not instant_ready:
            self.trigger_ready_since = None
            self.trigger_ready_announced = False
            return False, reason
        if self.trigger_ready_since is None:
            self.trigger_ready_since = now
        stable_elapsed = now - self.trigger_ready_since
        if stable_elapsed < self.config.trigger_stable_hold_s:
            return False, (
                f"stable hold {stable_elapsed:.2f}/"
                f"{self.config.trigger_stable_hold_s:.2f}s"
            )
        if not self.trigger_ready_announced:
            print("[READY] one-shot backflip trigger conditions are stable")
            self.trigger_ready_announced = True
        return True, "ready"
    ...
    def step(self):
        ...
        ready, reason = self._update_trigger_readiness(projected_gravity)

        if self.remote.is_pressed(KeyMap.A):
            if not self.allow_backflip:
                print("[A] trigger rejected: --wait-only is active")
            elif ready:
                self.phase_step = 0
                self.backflip_active = True
                self.flip_triggered_once = True
                self.trigger_ready_since = None
                self.trigger_ready_announced = False
        ...
        max_action_delta = (
            self.config.target_velocity_limits
            * self.config.control_dt
        ...
        target = np.clip(
            target,
            self.config.target_lower_limits,
            self.config.target_upper_limits,
        )
        ...
        if self.verbose or self.counter % 25 == 0:
            ...
            gate = "ready" if ready else reason
            print(
                f"step {self.counter:6d} | {mode} | phase {phase_time:4.2f}s | "
                f"tilt {tilt:4.2f} | maxdq {np.max(np.abs(self.dq)):5.2f} | "
                f"estimated torque ratio {torque_ratio:4.2f} | gate {gate}",
                flush=True,
            )
        if not self.allow_backflip and self.counter % 50 == 0:
            print(
                "[WAIT-Q] q="
                f"{np.array2string(self.q, precision=3)}",
                flush=True,
            )
        self.counter += 1
...
def check_only(config):
    ...
    observation[5] = -1.0
    # Waiting uses phase_duration=2 s: phase=(pi, pi/2, pi/4).
    observation[55] = -1.0
    observation[56] = 1.0
    observation[[58, 59]] = np.sqrt(0.5)
    ...
    print(
        "trigger: "
        f"one-shot={config.single_flip_only}, "
        "terminal-pose=tolerance-vector, "
        f"speed<={config.trigger_max_joint_speed:.2f} rad/s, "
        f"tilt<={config.trigger_max_tilt_rad:.2f} rad, "
        f"stable={config.trigger_stable_hold_s:.2f}s"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        help="enable terminal-phase standing policy but reject every A trigger",
    )
    ...
    controller = BackflipController(
        config, policy, sdk, verbose=args.verbose,
        allow_backflip=not args.wait_only,
    )
    ...
    try:
        ...
        if args.wait_only:
            print("[CONTROL] WAIT-ONLY: A is disabled | SELECT=damping stop")
        else:
