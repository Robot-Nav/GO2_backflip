...
当前已经完成实测的组合是 Unitree Go2 EDU、`enp131s0` 网卡、
`unitree_real` Conda 环境和
`outputs/go2_backflip_original_safe_v1/model_6000_actor.onnx`。主项目 README
中的“Go2 EDU 真机运行（已完成实测）”是现场操作清单，本文件说明内部实现。
...
python deploy_real/deploy_real_backflip.py \
  --check \
  --onnx outputs/go2_backflip_original_safe_v1/model_6000_actor.onnx
```
...
`--wait-only` 中按 A 也会被拒绝。看到 `[READY]` 后，先扶住机器人并用
`SELECT` 退出。该检查通过后，正式单次触发只需去掉 `--wait-only`。
...
3. 保持当前姿态 1 秒。
4. 用 8 秒余弦曲线进入训练默认 PD 目标姿态。
5. 保持 3 秒，按 hip/thigh/calf 的承重门限检查姿态并标定足端压力基线。
6. 等待手柄 `START`，不会自动启用 actor。
7. actor 启用后冻结在相位 2，等待触发。
8. 所有触发条件连续满足 1.0 秒后输出 `[READY]`；只接受一次 A。

训练配置中的 `default_angles` 是 PD 目标，不是承重后的实测关节角。当前
Go2 EDU 的终止相位参考来自 `--wait-only` 长时间稳定数据；每个关节允许
相对该参考偏差 `0.10 rad`。这避免用 MuJoCo 的静态平衡角或零动作目标误判
真机，同时仍保留速度、倾角、足底接触和连续稳定门控。
...
ARM：等待 START，使用电机端位置 PD 保持站姿
    |
    v
WAIT：启用 60 维 actor，phase=2，等待 READY
    |
    | A 且姿态、关节位置/速度和足端接触连续合格 1.0 s
    v
...
13.5 rad/s 后线性降额，在30 rad/s速度上限处降为零。
...
机器人位置、速度、actor 动作历史不会被清零。2 秒相位结束后，相位冻结在
2 秒，actor 继续负责落地和恢复。当前配置为 `single_flip_only=true`，第一次
成功触发后，本进程中的后续 A 都不会再触发动作。
...
曲线（13.5 rad/s 前保持 20.2/23.4 Nm 驱动/制动能力，在各关节速度上限
处降为零）。RL 阶段通过 `MotorCmd.tau` 发送最终限幅力矩，不再反算位置命令，
也不会让宇树电机端重复计算第二次 PD。
...
目标角会裁剪到 URDF 关节范围的 95%，与训练和 MuJoCo 一致。真机按每关节
速度上限检查，并对实测位置越过物理限位超过 0.02 rad 的情况进入阻尼停机。
...
部署层不再因机身倾角自动停机；RL 在相位结束后仍持续接管恢复。触发本进程
唯一一次空翻前仍要求机器人直立，`SELECT` 和通信看门狗始终生效。
...
4. 先运行 `--check`，再使用 `--wait-only` 完成归位、START 武装和 actor
   接管；该模式会强制禁用 A。
5. 必须看到 `[READY]`，并确认日志没有控制周期超时、关节异常或足端压力
   基线异常。
6. 正式进程中仍须等待最新状态为 `gate ready`，然后只触发一次 A。
7. 后空翻完成且机器人稳定后先扶住机身，再按 `SELECT`；退出会发送阻尼
   命令，并不会替机器人保持站姿。
