...
class BackflipConfig:
    def __init__(self, file_path, actor_override=None):
        ...
        self.joint_lower_limits = self._array(cfg, "joint_lower_limits")
        self.joint_upper_limits = self._array(cfg, "joint_upper_limits")
        self.soft_position_limit = float(cfg.get("soft_position_limit", 0.95))
        ...
        if "target_velocity_limits" in cfg:
            self.target_velocity_limits = self._array(
                cfg, "target_velocity_limits"
            )
        else:
            ...
            self.target_velocity_limits = (
                self.motor_velocity_limits * target_velocity_fraction
            )
        ...
        if "startup_joint_error_limits" in cfg:
            self.startup_joint_error_limits = self._array(
                cfg, "startup_joint_error_limits"
            )
        else:
            startup_max_joint_error = float(
                cfg.get("startup_max_joint_error", 0.08)
            )
            self.startup_joint_error_limits = np.full(
                12, startup_max_joint_error, dtype=np.float32
            )
        ...
        self.trigger_max_tilt_rad = float(cfg.get("trigger_max_tilt_rad", 0.10))
        self.trigger_max_joint_speed = float(
            cfg.get("trigger_max_joint_speed", 0.5)
        )
        if "trigger_joint_reference" in cfg:
            self.trigger_joint_reference = self._array(
                cfg, "trigger_joint_reference"
            )
        else:
            self.trigger_joint_reference = self.default_angles.copy()
        if "trigger_joint_reference_tolerances" in cfg:
            self.trigger_joint_reference_tolerances = self._array(
                cfg, "trigger_joint_reference_tolerances"
            )
        else:
            trigger_max_joint_position_error = float(
                cfg.get("trigger_max_joint_position_error", 0.08)
            )
            self.trigger_joint_reference_tolerances = np.full(
                12, trigger_max_joint_position_error, dtype=np.float32
            )
        self.trigger_stable_hold_s = float(
            cfg.get("trigger_stable_hold_s", 0.5)
        )
        self.single_flip_only = bool(cfg.get("single_flip_only", True))
        self.velocity_safety_ratio = float(
            cfg.get("velocity_safety_ratio", 1.0)
        )
        self.joint_position_tolerance = float(
            cfg.get("joint_position_tolerance", 0.02)
        )
        ...
        self.foot_contact_fraction = float(cfg.get("foot_contact_fraction", 0.8))
    ...
    def _validate(self):
        ...
        for name in (
            "default_angles", "joint_lower_limits", "joint_upper_limits",
            "target_velocity_limits", "kps", "kds", "torque_limits",
            "brake_torque_limits", "motor_velocity_limits",
            "startup_joint_error_limits", "trigger_joint_reference",
            "trigger_joint_reference_tolerances",
        ):
        ...
        if np.any(self.target_velocity_limits <= 0.0):
            raise ValueError("target_velocity_limits 必须为正数")
        if np.any(self.joint_lower_limits >= self.joint_upper_limits):
            raise ValueError("关节下限必须小于上限")
        if np.any(self.default_angles < self.joint_lower_limits) or np.any(
            self.default_angles > self.joint_upper_limits
        ):
            raise ValueError("default_angles 超出关节限位")
        if not 0.0 < self.soft_position_limit <= 1.0:
            raise ValueError("soft_position_limit 必须在 (0, 1] 内")
        ...
        if not 0.0 < self.velocity_safety_ratio <= 1.0:
            raise ValueError("velocity_safety_ratio 必须在 (0, 1] 内")
        if self.joint_position_tolerance < 0.0:
            raise ValueError("joint_position_tolerance 不能为负数")
        if np.any(self.startup_joint_error_limits <= 0.0):
            raise ValueError("startup_joint_error_limits 必须全部为正数")
        if self.trigger_max_tilt_rad <= 0.0:
            raise ValueError("trigger_max_tilt_rad 必须为正数")
        if self.trigger_max_joint_speed <= 0.0:
            raise ValueError("trigger_max_joint_speed 必须为正数")
        if np.any(self.trigger_joint_reference_tolerances <= 0.0):
            raise ValueError(
                "trigger_joint_reference_tolerances 必须全部为正数"
            )
        if self.trigger_stable_hold_s < 0.0:
            raise ValueError("trigger_stable_hold_s 不能为负数")
        if not 0.0 < self.foot_contact_fraction <= 1.0:
            raise ValueError("foot_contact_fraction 必须在 (0, 1] 内")
        ...
        ) * self.soft_position_limit
        self.target_lower_limits = midpoint - half_range
        self.target_upper_limits = midpoint + half_range
