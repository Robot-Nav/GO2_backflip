...
TARGET_VELOCITY_LIMITS = np.full(ACTION_DIM, 13.5, dtype=np.float32)
MOTOR_VELOCITY_X1 = 13.5
MOTOR_VELOCITY_X2 = JOINT_VELOCITY_LIMITS
...
def main():
    ...
    joint_midpoints = 0.5 * (joint_ranges[:, 0] + joint_ranges[:, 1])
    joint_half_ranges = 0.475 * (joint_ranges[:, 1] - joint_ranges[:, 0])
    target_lower_limits = joint_midpoints - joint_half_ranges
    target_upper_limits = joint_midpoints + joint_half_ranges
    ...
    # Terminal phase is the unambiguous waiting/standing command.  Phase zero
    # is reserved for the instant a backflip is triggered.
    phase_time = 0.0 if backflip_active else PHASE_DURATION
    policy_step = 0
    next_auto_trigger = (
        ...
        with viewer_context as viewer:
            while viewer.is_running() and keyboard_state.running:
                ...
                for _ in range(DECIMATION):
                    ...
                    target_pos = np.clip(
                        target_pos, target_lower_limits, target_upper_limits
                    )
                    ...
                    max_joint_speed_ratio = max(
                        max_joint_speed_ratio,
                        float(np.max(
                            np.abs(step_dof_vel) / JOINT_VELOCITY_LIMITS
                        )),
                    )
    ...
    print(
        ...
        f"最大速度倍率 {max_joint_speed_ratio:.3f} | "
        f"最大限位超出 {max_joint_limit_excess:.4f} rad | "
        f"头部接触 {'是' if head_contact else '否'}"
    )
    final_joint_pos = data.qpos[qpos_addresses]
    print(
        "诊断：最终关节位置 "
        f"{np.array2string(final_joint_pos, precision=3)}"
    )
    print(
        "诊断：最终默认姿态偏差 "
        f"{np.array2string(final_joint_pos - DEFAULT_DOF_POS, precision=3)}"
    )
