...
class Controller:
    ...
    def check_operator_and_link(self):
        if self.remote_controller.button[KeyMap.select] == 1:
            raise OperatorStop("SELECT pressed")
        if time.monotonic() - self.last_state_time > 0.10:
            raise OperatorStop("LowState timeout")
    ...
    def stand_only_state(self):
        print("Stand-only hold active. Press SELECT or Ctrl-C to damp and exit.")
        while True:
            self.check_operator_and_link()
            for i in range(12):
                motor_idx = self.config.joint2motor_idx[i]
    ...
    def damping_stop(self, duration=1.0):
        print(f"Sending damping commands for {duration:.1f} s.")
        create_damping_cmd(self.low_cmd)
        for _ in range(max(1, int(duration / self.config.control_dt))):
            self.send_cmd(self.low_cmd)
            time.sleep(self.config.control_dt)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stand-only",
        action="store_true",
        help="move to the default pose, hold it, and never enable the RL policy",
    )
    ...
    try:
        ...
        stop_reason = "Ctrl-C"
    except (OperatorStop, TimeoutError) as exc:
        stop_reason = str(exc)
    finally:
