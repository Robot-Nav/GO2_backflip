...
class Agent(AgentBase):
    ...
    def save(self, model_num):
        # 保存归一化参数（obs/state/reward 合并为单个文件）
        normalizer_dict = {
            'obs': self.obs_rms.state_dict(),
            'state': self.state_rms.state_dict(),
            'reward': self.reward_rms.state_dict(),
        }
        os.makedirs(self.save_dir, exist_ok=True)
        with open(f"{self.save_dir}/normalizer_{model_num}.pkl", 'wb') as f:
        ...
        torch.save(save_dict, f"{self.save_dir}/model_{model_num}.pt")
        cprint(f'[{self.name}] save success.', bold=True, color="blue")

    def load(self, model_num):
        # 加载归一化参数
        normalizer_file = f"{self.save_dir}/normalizer_{model_num}.pkl"
        if os.path.isfile(normalizer_file):
            with open(normalizer_file, 'rb') as f:
                normalizer_dict = pickle.load(f)
            self.obs_rms.load_state_dict(normalizer_dict['obs'])
            self.state_rms.load_state_dict(normalizer_dict['state'])
            self.reward_rms.load_state_dict(normalizer_dict['reward'])

        # 加载网络模型
        checkpoint_file = f"{self.save_dir}/model_{model_num}.pt"
