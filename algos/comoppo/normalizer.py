...
class RewardRMS(object):
    ...
    def state_dict(self):
        return {'mean': self.mean.cpu().numpy(), 'var': self.var.cpu().numpy(), 'count': self.count.cpu().numpy()}

    def load_state_dict(self, d):
        self.mean[:] = torch.tensor(d['mean'], dtype=torch.float32, device=self.device)
        self.var[:] = torch.tensor(d['var'], dtype=torch.float32, device=self.device)
        self.count[:] = torch.tensor(d['count'], dtype=torch.int64, device=self.device)
...
class ObsRMS(object):
    ...
    def state_dict(self):
        return {'mean': self.mean.cpu().numpy(), 'var': self.var.cpu().numpy(), 'count': self.count}

    def load_state_dict(self, d):
        self.mean[:] = torch.tensor(d['mean'], dtype=torch.float32, device=self.mean.device)
        self.var[:] = torch.tensor(d['var'], dtype=torch.float32, device=self.var.device)
        self.count = d['count']
        self.upgrade()
